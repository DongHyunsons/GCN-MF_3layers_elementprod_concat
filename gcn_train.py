...
import numpy as np
from SModel import SGCN_MFConcat
import time
import os
import copy
import sys
...
flags.DEFINE_integer('batch_size', 5, 'Number of batch size.')
...
def main(data_name, fold):
    ...
    #label_weights = np.zeros((user_count,item_count))                 ###   original source
    #weight_negative = 0.01                                            ###
    #label_weights = label_weights + weight_negative                   ###
    
    omim = pd.read_csv('genePhene.txt',header=None)
    omim = omim.values 
    omim = omim[:,0:3209].T
    
    for i in range(tr_data.shape[0]):
        labels[tr_data[i,0],tr_data[i,1]] = 1
        #label_weights[tr_data[i,0],tr_data[i,1]]=1                             #############   original source 
    
    Disgenet = pd.read_csv('data/DisGenet_InomimRange_scores.csv',index_col = 0)
    Disgenet = Disgenet.values.T[0:3209,:]
    
    trainDisgenet = np.clip(Disgenet - omim,0,1) + labels                  ##### 처음 고친 코드
    #trainDisgenet = np.clip(Disgenet + labels ,0,1)                          ##### 두번째 테스트    두번째 세번째는 테스트 데이터도 치팅할수 있도록 돼 있음
    #trainDisgenet = np.clip(Disgenet + omim  ,0,1)                          #####   세번째 테스트
    
    
    label_weights = copy.deepcopy(trainDisgenet)
        
    label_weights[label_weights>0] = 1
    
    labels = trainDisgenet
    
    
    label_weights = np.clip(label_weights + 0.01 , 0,1)
